...
int main() {
	...
	stdout = &uart_output;
	_delay_ms(10);
	//Desabilita interrupção do TIMER 2
	TIMSK2 &= ~_BV(OCIE2A);
	...
	while (1) {
		...
		//Cartão encontrado
		//Verifica Se o cartão esta registrado
		for (uint8_t i = 0; i < 2; i += 1) {
			...
			//Compara se o gato ja pode ser alimentado pelo tempo dês de a ultima alimentação
			...
			//Liga o giro do motor ate chegar no peso de ração adequado
			TIMSK2 |= _BV(OCIE2A);
			while (1) {
				int32_t weight = hx711_read_average();
				printf("Current weight = %li\n\n", weight);

				if (weight >= refill_quantities[refill_target]) {
					break;
				}
			}
			TIMSK2 &= ~_BV(OCIE2A);
